...
Intent includes the program digest, snapshot digest, resource, and full plan. Reusing an operation identity with a different intent raises an error. A receipt's `commit_id` is the full SHA-256 digest of the operation identity, independent of the intent digest. The runtime currently uses the deterministic default; durable caller operation IDs are groundwork for Day 5.
...
Data and receipt live in the same local transaction. This does not establish exactly-once behavior for external APIs, network filesystems, hardware failures outside SQLite's durability model, or manually altered ledgers. Crash reconciliation is the next milestone.
